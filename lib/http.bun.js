export class HttpMux {
    ...
    strip_prefix = (strip_prefix = "") => {
        ...
        return async (request, server) => {
            ...
            if (server) request.ip = server.requestIP(request);

            // get client ip and port from nginx via "X-Forwarded-For" and "x-real-ip" header
            if (request.ip?.address) request.ip.address = request.headers.get("X-Forwarded-For") || request.ip.address;
            if (request.ip?.port) request.ip.port = request.headers.get("x-real-port") || request.ip.port;
